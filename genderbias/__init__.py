...
def scanned_detectors():
    ...
    module_paths = [path for path in all_full_module_paths
                    if os.path.isdir(path) and os.path.basename(path) not in exclude_dirs]
    module_path_to_module = {path: importlib.import_module("."+os.path.basename(path), "genderbias")
                             for path in module_paths}
    return [(module_path, class_name, cls)
